define(function(require, exports, module) {
	...
	$.parser={parseOptions: function(_6, _7) {
					var t = $(_6);
					var _8 = {};
			...
			$.fn._outerWidth = function(_e) {
				...
				return this.each(function() {
					if (!$.support.boxModel && BROWSER.ie) {
						$(this).width(_e);
					} else {
	...
	function init(target) {
		...
		$(target).on('change','#select_month',function () {
			showMonth(target, $(this).val());
		});

		$(target).on('change', '#select_year', function () {
			showYear(target, $(this).val());
		});
	...
	function show(target) {
		...
		var selectYear = '<select id="select_year">';
		for (var i=-3; i<2; i++) {
			if (nowYear+i == opts.year) {
				selectYear += '<option value="'+(nowYear+i)+'" selected="selected">'+(nowYear+i)+'</option>';
			}else {
				selectYear += '<option value="'+(nowYear+i)+'">'+(nowYear+i)+'</option>';
			}
		}
		selectYear += '</select>';
		var selectMonth = '<select id="select_month">';
		for (var i=1;i < 13; i++) {
			if (i == (opts.month)) {
				selectMonth += '<option value="'+i+'" selected="selected">'+i+'月</option>';
			}else {
				selectMonth += '<option value="'+i+'">'+i+'月</option>';
			}
		}
		selectMonth += '</select>';
		$(target).find('.calendar-title').html(selectYear + selectMonth + '<span>农历' + cyclical(opts.year - 1900 + 36) + '年(' + Animals[(opts.year - 4) % 12] + ')');
			...
			for (var j = 0; j < week.length; j++) {
				...
				if (opts.month == day[1]) {
					...
					if (info.color) {
						...
						dayHtml = '<font' + color + '>' + day[2];
					}
					dayHtml += '</font>';
					if (opts.lunarDay) {
						if (opts.solarTerms && info.solarTerms) {
							dayHtml += '<font>' + info.solarTerms + '</font>';
						} else {
							dayHtml += '<font>' + cDay(info.lDay, info.lMonth) + '</font>';
						}
					}
				}
//				if (dayHtml)
		...
		this.each(function () {
			if (state) {
				...
				state = $.data(this, 'fullCalendar', {
						options : $.extend({}, $.fn.fullCalendar.defaults,$.fn.fullCalendar.parseOptions(this), options)
					});
				init(this);
			}
	...
	//增加样式
	$('head').append('<style>.calendar-today{background:red;}.fullcalendar-day {border-left: 1px dotted green;border-top: 1px dotted green;} '
		 + '.calendar-hover{border: 1px solid red;}.fullcalendar-body th{padding: 5px;height:20px;}'
		 + '.calendar-selected {border: 1px solid red;} td.calendar-other-month{visibility: hidden;}</style>');
});
