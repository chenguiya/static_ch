var starInfo={
	...
	'origTxt':function(btn,orig){
	  if(orig.is(':hidden')){
	     orig.slideDown();  //slideDown意思从隐藏到可见的速度
	  }else{
	     orig.slideUp();
	  }
	...
	'comment':function(triggerEl,commentBox,relativeClass){
	  ...
	  if(triggerEl.siblings('.zf').hasClass('zf_ed')){
	     starInfo.share(triggerEl.siblings('.zf'),triggerEl.parent().parent().siblings('.shareMain'));
	  }
	},
	//分享
...
$(function() {
       //头部搜索    其中:root 选择器匹配文档根元素。在 HTML 中，根元素始终是 html 元素。
	   ...
	   $('.infoBox').on('click','.textbg',function(){//原文
	     starInfo.origTxt($(this),$(this).parents('.infoBg').find('.infoCon_all'));
	   }).on('click','.listSetBox .funBox .pl',function(){//右侧评论展示
